...
```bash
curl -sSL "<URL_SCRIPT>" -o /tmp/ovh-ba-install.sh && chmod +x /tmp/ovh-ba-install.sh && sudo bash /tmp/ovh-ba-install.sh --setup "<URL_MANAGEMENT_AGENT>"
```

Remplacer :
- `<URL_SCRIPT>` : URL du script `ovh-ba-install.sh` (hébergé par vous)
...
```bash
curl -sSL "<URL_SCRIPT>" -o /tmp/ovh-ba-install.sh && chmod +x /tmp/ovh-ba-install.sh && sudo bash /tmp/ovh-ba-install.sh --setup-local
```
...
- **Menu seul** : `sudo bash ovh-ba-install.sh` ou `sudo ovhbackupagent` (si déjà installé)
- **Installation Management Agent** (URL ou chemin) : `sudo bash ovh-ba-install.sh "https://.../LinuxAgentPackages.xxx.sh"`
- **Setup complet** (mgmt + ovhbackupagent + README + menu) : `sudo bash ovh-ba-install.sh --setup "https://.../LinuxAgentPackages.xxx.sh"`
- **Script seul (agents déjà installés)** : `sudo bash ovh-ba-install.sh --setup-local` (pas d’URL)
- **Diagnostic** : `sudo bash ovh-ba-install.sh --diagnostic`
- **Installer uniquement la commande ovhbackupagent** : `sudo bash ovh-ba-install.sh --install-global`

## Prérequis

- `curl`, `bash`, droits root (`sudo`)

README intégré (accessible via le menu ou `sudo bash ovh-ba-install.sh --readme`) : `sudo bash ovh-ba-install.sh --readme`
